...
const RegisterForm = ({ sendToLogin,setNotification,logIn }) => {
  ...
  const handleRegister = (e) => {
    ...
    if(!confirmPasswordInput.value)  return setNotification("Enter confirm password");
    if(!dateOfBirthInput.value)  return setNotification("Enter date of birth");
    if(passwordInput.value !== confirmPasswordInput.value) return setNotification("Passwords do not match");
    const age = new Date().getFullYear() - new Date(dateOfBirthInput.value).getFullYear();
